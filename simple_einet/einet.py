...
class Einet(nn.Module):
    ...
    def forward(self, x: torch.Tensor, marginalized_scopes: torch.Tensor = None) -> torch.Tensor:
        ...
        if hasattr(self, "permutation"):
            assert self.config.num_repetitions == 1, "Permutation is not supported for multiple repetitions."
    ...
    def _build_structure_bottom_up(self):
        ...
        self.register_buffer("permutation", torch.randperm(self.config.num_features))
        self.register_buffer("permutation_inv", invert_permutation(torch.tensor(self.permutation)))
    ...
    def _build_input_distribution_bottom_up(self) -> FactorizedLeaf:
    ...
    def sample(
        ...
        with sampling_context(self, evidence, marginalized_scopes, requires_grad=is_differentiable, seed=seed):
            ...
            if hasattr(self, "latent_layer"):
                ctx = self.latent_layer.sample(ctx=ctx)
            ...
            if hasattr(self, "permutation_inv"):
                ctx.indices_out = ctx.indices_out[:, self.permutation_inv]
            ...
            if evidence is not None and fill_evidence:
