...
def index_one_hot_fast(tensor: torch.Tensor, index: torch.Tensor, dim: int) -> torch.Tensor:
    """
    Index into a given tensor unsing a one-hot encoded index tensor at a specific dimension.
    ...
    if False:
        ...
        return torch.sum(tensor * index, dim=dim)
        # tensor_expanded = tensor.expand(index.shape[0], -1, -1, -1, -1)
        # rng = torch.arange(0, index.shape[dim], device=tensor.device).view(1, -1, 1, 1, 1).expand_as(index)
        # index_disc = torch.sum(index * rng, dim=dim)
        # index_disc = index_disc.unsqueeze(2).expand(-1, -1, tensor_expanded.shape[2], -1, -1)  # Make space for input channels
        # res = tensor_expanded.gather(dim=dim, index=index_disc.long())

        # # Straight through estimator
        # res = (res - index_disc).detach() + index_disc
