...
def dist_forward(distribution, x: torch.Tensor):
    ...
    try:
        ...
        logger.info("min:", x.min())
        logger.info("max:", x.max())
        raise e
...
def dist_mode(distribution: dist.Distribution, ctx: SamplingContext = None) -> torch.Tensor:
    ...
    from simple_einet.layers.distributions.categorical import CustomCategorical
    from simple_einet.layers.distributions.binomial import DifferentiableBinomial
    from simple_einet.layers.distributions.bernoulli import DifferentiableBernoulli
    ...
    if isinstance(distribution, CustomNormal):
        ...
        if mode.shape[0] == 1:
            ...
            return mode
    elif isinstance(distribution, dist.Bernoulli) or isinstance(distribution, DifferentiableBernoulli):
        mode = distribution.probs.clone()
        mode[mode >= 0.5] = 1.0
        mode[mode < 0.5] = 0.0
        return mode.repeat(ctx.num_samples, 1, 1, 1, 1)
        ...
        return mode.repeat(ctx.num_samples, 1, 1, 1, 1)
    elif isinstance(distribution, CustomCategorical):
        return distribution.mpe(num_samples=ctx.num_samples)
    else:
...
def dist_sample(distribution: dist.Distribution, ctx: SamplingContext = None) -> torch.Tensor:
    ...
    if (ctx.is_mpe or ctx.mpe_at_leaves) and not ctx.return_leaf_params:
        ...
        if ctx.return_leaf_params:
            ...
            # Some distributions support temperature scaling directly
            if type(distribution) in (DifferentiableBinomial, DifferentiableBernoulli):
                samples = distribution.sample(sample_shape=(ctx.num_samples,), tau=ctx.temperature_leaves).float()
            else:
