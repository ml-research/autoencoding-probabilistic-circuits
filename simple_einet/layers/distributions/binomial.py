...
class DifferentiableBinomial:
    ...
    def sample(self, sample_shape: Tuple[int], tau=1.0):
        """
...
if __name__ == '__main__':
    ...
    N = 1
    binomial = Binomial(num_features, num_channels, num_leaves, num_repetitions, total_count=N)

    # Test forward pass
    x = torch.randint(0, N + 1, size=(5, num_channels, num_features)).float()
    log_likelihood = binomial(x, marginalized_scopes=None)
    assert log_likelihood.shape == (5, num_channels, num_features, num_leaves, num_repetitions)
    print('Binomial test successful!')

    # LLS: Check if gradients work w.r.t. the parameters
    binomial.zero_grad()
    log_likelihood = binomial(x, marginalized_scopes=None)
    log_likelihood.sum().backward()
    print(binomial.logits.grad.sum())
    ...
    samples = binomial.sample(ctx=ctx)
    samples.sum().backward()
    print(binomial.logits.grad)
